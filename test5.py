...
def create_combined_training_data(data, denoised_features, train_indices,
    ...
    # 创建组合训练数据 (包含四种边：原始-原始，原始-扩散，扩散-原始，扩散-扩散)
    combined_train_data = []
    ...
    # 2. 扩散节点之间的边（保持相同的连接模式和距离）
    for i in range(len(train_indices)):
        src = original_src_nodes_train[
                  i].item() + diffusion_offset  # 添加偏移以表示扩散节点
    ...
    for i in range(len(test_indices)):
        ...
        combined_test_data.append((src, dst, dist, "original-original"))

    # 2. 扩散节点之间的边
    for i in range(len(test_indices)):
        src = original_src_nodes_test[i].item() + diffusion_offset
        dst = original_dst_nodes_test[i].item() + diffusion_offset
        dist = original_true_distances_test[i].item()
        combined_test_data.append((src, dst, dist, "diffusion-diffusion"))

    return combined_train_data, combined_test_data, diffusion_offset
...
def train_distance_predictor_with_combined_data(model, combined_train_data,
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 5 == 0:
            ...
            # 单独评估原始节点和扩散节点性能
            test_loss_original = evaluate_model_with_combined_data(
                model=model,
                combined_test_data=[d for d in combined_test_data if
                                    d[3] == "original-original"],
                node_to_feature=node_to_feature,
                device=device,
                batch_size=batch_size
            )

            test_loss_diffusion = evaluate_model_with_combined_data(
                model=model,
                combined_test_data=[d for d in combined_test_data if
                                    d[3] == "diffusion-diffusion"],
                node_to_feature=node_to_feature,
                device=device,
                batch_size=batch_size
            )
            ...
            print(
                f"  Original Test Loss = {test_loss_original:.6f}, Diffusion Test Loss = {test_loss_diffusion:.6f}")
            ...
            if test_loss < best_test_loss:
                ...
                torch.save({
                    ...
                    'test_loss_diffusion': test_loss_diffusion,
                }, "best_distance_predictor_combined_by_test.pt")
                print(
        ...
        if avg_train_loss < best_train_loss:
            ...
            }, "best_distance_predictor_combined_by_train.pt")
            print(
    ...
    if (num_epochs - 1) % 5 != 0:
        ...
        final_test_loss_original = evaluate_model_with_combined_data(
            model=model,
            combined_test_data=[d for d in combined_test_data if
                                d[3] == "original-original"],
            node_to_feature=node_to_feature,
        ...
        final_test_loss_diffusion = evaluate_model_with_combined_data(
            model=model,
            combined_test_data=[d for d in combined_test_data if
                                d[3] == "diffusion-diffusion"],
            node_to_feature=node_to_feature,
        ...
        print(
            f"  Original Test Loss = {final_test_loss_original:.6f}, Diffusion Test Loss = {final_test_loss_diffusion:.6f}")

    print(f"Combined distance predictor training completed.")
    print(
...
if __name__ == "__main__":
    ...
    # 为所有节点生成去噪特征 - 使用修改后的函数返回节点特征映射和所有去噪特征
    ...
    # 创建组合训练和测试数据
    combined_train_data, combined_test_data, diffusion_offset = create_combined_training_data(
        data=data,
    ...
    # 评估最终模型 - 分别评估原始节点和扩散节点
    final_mse_original = evaluate_model_with_combined_data(
        model=model,
        combined_test_data=[d for d in combined_test_data if
                            d[3] == "original-original"],
        node_to_feature=combined_node_to_feature,
    ...
    final_mse_diffusion = evaluate_model_with_combined_data(
        model=model,
        combined_test_data=[d for d in combined_test_data if
                            d[3] == "diffusion-diffusion"],
        node_to_feature=combined_node_to_feature,
    ...
    final_mse_combined = evaluate_model_with_combined_data(
        model=model,
        combined_test_data=combined_test_data,
        node_to_feature=combined_node_to_feature,
        device=device,
    ...
    print(f"Final model evaluation - Original MSE: {final_mse_original:.6f}")
    print(f"Final model evaluation - Diffusion MSE: {final_mse_diffusion:.6f}")
    print(f"Final model evaluation - Combined MSE: {final_mse_combined:.6f}")

    # 保存完整模型
    torch.save(model.state_dict(), "final_combined_node_diffusion_model.pt")
