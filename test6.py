...
# class EnhancedDistancePredictor(nn.Module):
#     """增强版距离预测网络 - 接受连接的节点特征对并添加位置编码"""
#
#     def __init__(self, node_dim, hidden_dim=512, depth=1, max_len=1000,
#                  dropout=0.4):
#         super().__init__()
#         # 计算两个节点特征连接后的维度
#         input_dim = node_dim * 2
...
#
#         # 获取位置ID，如果没有提供则使用默认序列
#         if position_ids is None:
#             # 创建0到batch_size-1的序列，并确保不超过最大长度
#             position_ids = torch.arange(batch_size, device=x.device)
#             position_ids = torch.clamp(position_ids, max=self.position_embeddings.num_embeddings-1)
#
#         # 获取位置编码
#         position_embeddings = self.position_embeddings(position_ids)
#
#         # 将位置编码与投影后的输入相加
#         x = projected_x + position_embeddings
#
#         # 归一化和dropout
#         x = self.LayerNorm(x)
#         x = self.dropout(x)
...
#         x = self.layers[-1](x)
#
#         # 确保输出是一维的[batch_size]形状
#         return x.view(batch_size)  # 强制调整为[batch_size]形状

class GaussianDiffusion:
    """实现高斯扩散过程，包括前向加噪和反向去噪"""

    def __init__(self, num_timesteps=1000, beta_start=1e-6, beta_end=0.0001):
...
def evaluate_model_with_combined_data(model, combined_test_data,
    ...
    return mse, pearson_r, pearson_p
    # return mse
...
def train_distance_predictor_with_combined_data(model, combined_train_data,
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 2 == 0:
            ...
            test_loss, test_r, test_p = evaluate_model_with_combined_data(
                model=model,
            ...
            if test_loss < best_test_loss:
                ...
                print(
                    f"  New best test loss: {best_test_loss:.6f} (仅供参考，不保存模型)")
...
if __name__ == "__main__":
    ...
    # encoder_model.load_state_dict(torch.load('best_encoderH3N2_model.pth'))
    with torch.no_grad():
    ...
    final_mse, final_r, final_p = evaluate_model_with_combined_data(
        model=model,
        combined_test_data=test_data,  # 只包含原始数据的测试集
        node_to_feature=combined_node_to_feature,
        device=device,
    ...
    print(
        f"Final model evaluation - MSE: {final_mse:.6f}, Pearson r: {final_r:.4f}, p-value: {final_p:.4e}")
