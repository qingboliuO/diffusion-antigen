...
class EnhancedDistancePredictor(nn.Module):
    ...
    def __init__(self, node_dim, hidden_dim=512, depth=1, nhead=8,
    ...
    def forward(self, x):
        ...
        # 应用Transformer编码器
        # 转换为序列格式 [batch_size, 1, hidden_dim]
        x_seq = x.unsqueeze(1)
        transformed = self.transformer_encoder(x_seq)
        x = transformed.squeeze(1)  # 转换回 [batch_size, hidden_dim]
