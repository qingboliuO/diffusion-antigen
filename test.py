...
class EnhancedDistancePredictor(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim=512, depth=4):
        super().__init__()
        ...
        for _ in range(depth):
            layers.append(ResidualBlock(hidden_dim // 2, hidden_dim // 4))
...
class DenoiseNet(nn.Module):#去噪网络，学习预测和去除噪声
    """去噪模型"""
...
class DiffusionDistancePredictor(nn.Module):#完整模型，结合扩散过程和距离预测
    ...
    def __init__(self, input_dim, diffusion_steps=300, beta_start=1e-4,
        ...
        # 去噪网络
        self.denoise_net = DenoiseNet(input_dim=input_dim,
...
class GraphAntigenEnv:
    ...
    def step(self, predicted_distances):
        # 更新索引
    ...
    with torch.no_grad():
        for i in range(0, len(test_indices), batch_size):
            ...
            t = torch.full((batch.shape[0],), diffusion_steps, device=device,
                           dtype=torch.long)
            noisy_batch = model.forward_diffusion(batch, t)

            # 去噪
            denoised_batch = model.denoise(noisy_batch, diffusion_steps)
...
if __name__ == "__main__":
    ...
    batch_size = 256
    # 使用OneCycleLR替代ExponentialLR
    # 计算每个epoch的步数
    steps_per_epoch = len(train_idx) // batch_size + 1
    # 设置总步数
    total_steps = steps_per_epoch * 1000  # 假设1000个epoch
    ...
    scheduler = OneCycleLR(
        ...
        pct_start=0.3,  # 学习率上升阶段占总步数的30%
        anneal_strategy='cos',  # 使用余弦退火
        div_factor=25.0,  # 初始学习率为max_lr/25
        final_div_factor=1e4  # 最终学习率为初始学习率/10000
    )
    ...
    for epoch in range(1000):
        ...
        while batch is not None:
            ...
            # 计算去噪损失
            denoise_loss, _ = model.get_loss(batch, t)

            # 第二阶段：使用去噪后的embedding预测距离
            # 选择一个固定时间步添加噪声
            fixed_t = torch.full((batch.shape[0],), diffusion_steps // 2,
                                 device=device, dtype=torch.long)
            noisy_batch = model.forward_diffusion(batch, fixed_t)
            ...
            # 使用去噪后的特征预测距离
            pred_distances = model.predict_distance(denoised_batch)
            true_distances = data.edge_attr[batch_indices].to(device)

            # 计算距离预测损失
            pred_loss = nn.MSELoss()(pred_distances, true_distances)

            # 总损失：平衡去噪损失和预测损失
            loss = denoise_loss + pred_loss
            ...
            # 注意：对于OneCycleLR，应在每个批次后调用scheduler.step()
            scheduler.step()
            ...
            # 进入下一批次
            batch, batch_indices, done = train_env.step(pred_distances)
            if done:
                break
        ...
        # 注意：删除这里的scheduler.step()，因为我们已经在每个批次后调用了
